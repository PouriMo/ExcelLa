...
Private Sub btnExportThrust_Click()
    Dim savePath As Variant
    Dim ws As Worksheet
    Dim chObj As ChartObject
    
    On Error Resume Next
    Set ws = UserWorkbook().Sheets("ThrustPlot")
    On Error GoTo 0
    
    If ws Is Nothing Then
        MsgBox "Thrust line plot not found. Run the analysis first.", vbExclamation
        Exit Sub
    End If
    ...
    savePath = Application.GetSaveAsFilename( _
        InitialFileName:="ThrustLine.png", _
        FileFilter:="PNG Image (*.png), *.png", _
        Title:="Save Thrust Line Plot")
        
    If savePath <> False Then
        chObj.Chart.Export Filename:=CStr(savePath), FilterName:="PNG"
        MsgBox "Thrust line plot saved successfully to:" & vbCrLf & savePath, vbInformation
    End If
End Sub

Private Sub btnExportDeformed_Click()
    Dim savePath As Variant
    Dim ws As Worksheet
    Dim chObj As ChartObject
    
    On Error Resume Next
    Set ws = UserWorkbook().Sheets("DeformedPlot")
    On Error GoTo 0
    
    If ws Is Nothing Then
        MsgBox "Deformed shape plot not found. Run the analysis first.", vbExclamation
        Exit Sub
    End If
    ...
    savePath = Application.GetSaveAsFilename( _
        InitialFileName:="DeformedShape.png", _
        FileFilter:="PNG Image (*.png), *.png", _
        Title:="Save Deformed Shape Plot")
        
    If savePath <> False Then
        chObj.Chart.Export Filename:=CStr(savePath), FilterName:="PNG"
        MsgBox "Deformed shape plot saved successfully to:" & vbCrLf & savePath, vbInformation
    End If
End Sub
